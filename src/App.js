...
const App = () => {
	...
	const [generalIntro, setGeneralIntro] = useState( 
		{
	...
	const [skills, setSkills] = useState([['Web Dev', 'CAD'], false]);
	...
	const editField = (() => {
		...
		const hide = () =>{
			...
			let prevGeneralIntro = {...generalIntro}; // Get a copy of generalIntro object from the state variable
			// Set editStatus has false to hide editField on the DOM.
			for (const key in prevGeneralIntro) {
				prevGeneralIntro[key][1] = false;
			}
			
			let prevGeneralLink = {...generalLink}; // Get a copy of generalLink object from the state variable
			// Set editStatus has false to hide editField on the DOM.
			for (const key in prevGeneralLink) {
				prevGeneralLink[key][1] = false;
			}

			let prevWorkExp = [...workExperiences]; // Get a copy of workExperiences array from the state variable
			// Set editStatus has false to hide editField on the DOM.
			...
			let prevEducation = [...education]; // Get a copy of workExperiences array from the state variable
			// Set editStatus has false to hide editField on the DOM.
			...
			setGeneralIntro(prevGeneralIntro);
			setGeneralLink(prevGeneralLink);
			setSkills([skills[0], false]);
			setWorkExperiences(prevWorkExp);
		...
		const save = () =>{
			...
			switch (fieldSection) {
			case 'generalIntro': // For General Intro / General Link section: 
				prevObject = {...generalIntro}; // Get a copy of general object from the state variable
			...
			case 'generalLink': // For General Intro / General Link section: 
				prevObject = {...generalLink}; // Get a copy of general object from the state variable
			...
			case 'workExperiences':
				...
				prevObject= [...workExperiences]; // Get a copy of work experience object from the state variable
			...
			case 'education':
				...
				prevObject= [...education]; // Get a copy of work experience object from the state variable
	...
	const skill = (() => {
		...
		const saveSkill = () =>{
			...
			let prevSkillsList = [...skills]; // Get a copy of skills array from the state variable
		...
		const deleteSkill = (e) =>{			
			...
			let prevSkills = [...skills]; // Get a copy of skill array from the state variable
	...
	const workExperience = (() => {
		...
		const addWorkExperience = () =>{
			...
			let prevWorkExp= [...workExperiences]; // Get a copy of workExperience array from the state variable
	...
	const task = (() => {
		...
		const saveTask = () =>{
			...
			let prevWorkExp= [...workExperiences]; // Get a copy of work experience array from the state variable
		...
		const deleteTask = (e) =>{
			...
			let prevWorkExp= [...workExperiences]; // Get a copy of work experience object from the state variable
	...
	const edu = (() => {
		...
		const addEducation = () =>{
			...
			let prevEducation= [...education]; // Get a copy of education array from the state variable
	...
	const mode = (()=>{
		...
		const preview = async () => {
			...
			await editField.hide(); // If an edit field is open, hide it (await is needed to correctly apply attribute changes to the field)
	...
	return (
		<div className="App">
			<Header 
				onPreviewClick= { mode.preview } 
				onEditClick= { mode.edit }/>
			<div className="App-cv">
	  			<div className="general-container">
					<GeneralIntro  generalState={ generalIntro } 
						onFieldClick={ editField.show } 
						onSaveClick={ editField.hide } 
						onInput={ editField.save }  />
	  				<GeneralLink  generalState={ generalLink } 
						onFieldClick={ editField.show } 
						onSaveClick={ editField.hide } 
						onInput={ editField.save }/>
				...
				<Skills skillsList = {skills} 
					onAddSkill = { editField.show }
					onSaveClick={ skill.saveSkill } 
					onDeleteSkill={ skill.deleteSkill } 
				/>
				<Section sectionName="Work Experience" />
				<WorkExperience worksList={workExperiences} 
					onFieldClick={ editField.show } 
					onSaveClick={ editField.hide } 
					onInput={ editField.save }
					onAddWorkExperience={ workExperience.addWorkExperience }
					onDeleteWorkExperience={ workExperience.deleteWorkExperience }
					onEditTask={ editField.show } 
					onDeleteTask={ task.deleteTask } 
					onSaveTask={ task.saveTask }
				/>
				...
				<Education educationList={education}
					...
					onSaveClick={ editField.hide } 
					onInput={ editField.save } 
					onAddEducation={ edu.addEducation } 
					onDeleteEducation={ edu.deleteEducation }
				/>
			</div>
